...
computeMandelbrotSet( int W, int H, int maxIter )
{
    ...
    int Hloc = H / size;
    firstrow = (size-(rank+1)) * Hloc;
    endrow = (size-rank) * Hloc ; 
    ...
    // On parcourt les pixels de l'espace image :
    for ( int iglob = firstrow; iglob < endrow; ++iglob ) {
      int iloc = iglob - firstrow;
      computeMandelbrotSetRow(W, H, maxIter, iglob, pixels.data() + W*(Hloc-iloc-1) );
    }
...
int main(int argc, char *argv[] ) 
 { 
   ...
   const int W = 800;
    ...
    auto pixels_loc = computeMandelbrotSet( W, H, maxIter );
